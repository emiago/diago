...
func (m *MediaSession) Init(webrtcConfig webrtc.Configuration) error {
	...
	var err error
	if factoryValue, ok := rtcpInterceptorFactories.Load(api); ok {
		factory := factoryValue.(*rtcpInterceptorFactory)
		factory.createMu.Lock()
		createdBefore := factory.createdCount()
		peerConnection, err = api.NewPeerConnection(webrtcConfig)
		if rtcpInterceptor := factory.takeCreated(createdBefore); rtcpInterceptor != nil && err == nil {
			rtcpInterceptor.hooks.Store(m.rtcpHooks)
		}
		factory.createMu.Unlock()
	} else {
		peerConnection, err = api.NewPeerConnection(webrtcConfig)
	}
	if err != nil {
...
func (m *MediaSession) Fork() *MediaSession {
	return &MediaSession{
		...
		rtcpHooks:      m.rtcpHooks,
	}
}

// OnReadRTCP sets a callback for each RTCP packet received by this WebRTC
// session. Passing nil disables the callback.
func (m *MediaSession) OnReadRTCP(f func(pkt rtcp.Packet, rtpStats RTPReadStats)) {
	m.mu.Lock()
	if m.rtcpHooks == nil {
		m.rtcpHooks = newRTCPHooks()
	}
...
// OnWriteRTCP sets a callback for each RTCP packet sent by this WebRTC
// session, including reports generated by Pion. Passing nil disables it.
func (m *MediaSession) OnWriteRTCP(f func(pkt rtcp.Packet, rtpStats RTPWriteStats)) {
	m.mu.Lock()
	if m.rtcpHooks == nil {
		m.rtcpHooks = newRTCPHooks()
	}
...
func (m *MediaSession) setupTracks(log *slog.Logger) error {
	...
	peerConnection.OnTrack(func(remoteTrack *webrtc.TrackRemote, receiver *webrtc.RTPReceiver) { //nolint: revive
		...
		go func() {
			log.Debug("Webrtc reading receiver RTCP")
			defer log.Debug("Webrtc reading receiver RTCP stopped")
